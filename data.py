...
class DatasetForBert:
    ...
    def generate_datasets(self):
        ...
        raw_dataset = load_dataset(
            self.dataset_infor["file_type"], data_files=train_file, split="train"
        )
        if val_file:
            val_dataset = load_dataset(
                self.dataset_infor["file_type"], data_files=val_file, split="train"
            ).shuffle(seed=42)
            has_val = True
        if test_file:
            test_dataset = load_dataset(
                self.dataset_infor["file_type"], data_files=test_file, split="train"
            ).shuffle(seed=42)
            has_test = True
        if has_val and has_test:
            ...
            trainval_test = raw_dataset.train_test_split(test_size=0.15, seed=42)
            trainval = trainval_test["train"]
            test_dataset = trainval_test["test"]

            train_val = trainval.train_test_split(test_size=0.2, seed=42)
            train_dataset = train_val["train"]
            val_dataset = train_val["test"]
